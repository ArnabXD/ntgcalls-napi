...
`index.js` is an ESM wrapper that `require()`s the `.node` file and re-exports `NtgCalls`. `index.d.ts` is automatically generated by `@napi-rs/cli` during compilation. Do NOT edit `index.d.ts` manually, as it will be overwritten on the next build. All type adjustments (such as mapping native `i64` to TypeScript `bigint`) must be done in their respective submodule source files (e.g. `src/session.rs` or `src/types.rs`) using N-API macros (e.g. `#[napi(ts_type = "bigint")]`, `#[napi(ts_arg_type = "bigint")]`, or `#[napi(ts_return_type = "Promise<bigint>")]`).
